...
public class EquipmentDAOImpl implements EquipmentDAO {
    ...
    public List<Equipment> getAll() throws SQLException, ClassNotFoundException {
        ...
        ResultSet rs = SQLUtil.executeQuery("SELECT * FROM Equipment");
        while (rs.next()) {
    ...
    public  String generateEquipmentId() throws SQLException, ClassNotFoundException {
        ResultSet rst = SQLUtil.executeQuery("SELECT equipment_id FROM Equipment ORDER BY equipment_id DESC LIMIT 1");
        if (rst.next()) {
    ...
    public  boolean delete(String equipmentId) throws SQLException, ClassNotFoundException {
        return SQLUtil.executeUpdate("DELETE FROM Equipment WHERE equipment_id = ?", equipmentId);
    }
}
